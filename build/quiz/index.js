...
const Edit = ({
  ...
  const removeQuestion = indexToRemove => {
    ...
    const newCorrectAnswers = correctAnswers.filter((_, index) => index !== indexToRemove);
    setAttributes({
      questions: newQuestions,
  ...
  const removeAnswer = (questionIndex, answerIndex) => {
    const newQuestions = questions.map((q, i) => i === questionIndex ? {
      ...q,
    ...
    const newCorrectAnswers = correctAnswers.map((ca, i) => i === questionIndex && answerIndex < ca ? ca - 1 : ca);
    setAttributes({
      questions: newQuestions,
      correctAnswers: newCorrectAnswers
    });
  ...
  const setCorrectAnswer = (questionIndex, answer) => {
    ...
    setAttributes({
      correctAnswers: correctAnswers.map((ca, i) => i === questionIndex ? newCorrectAnswer : ca)
    });
  };
